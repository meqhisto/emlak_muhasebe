...
import { BarChart3, AlertTriangle, Wallet, Calendar, Printer, Activity } from 'lucide-react';

const Reports: React.FC = () => {
<<<<<<< bolt-performance-optimizations-11727286907015454391
    // Data State - now from Context
    const { transactions, expenses, consultants } = useData();
...
=======
  ...
  const calculatePartnerStats = (payerType: Payer) => {
    ...
    const paidExpenses = filteredExpenses.filter(e => e.paidBy === payerType && e.isPaid).reduce((acc, e) => acc + e.amount, 0);
    const shareOfExpenses = totalExpenses / 2;
    const netProfitShare = incomeShare - shareOfExpenses;
    return { grossShare: incomeShare, paidExpenses, shareOfExpenses, netProfitShare, totalBalance: netProfitShare + paidExpenses };
  };
...
>>>>>>> main
  ...
  const renderPartners = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in duration-300 print:block print:space-y-6">
        ...
        <div className="col-span-1 md:col-span-2 bg-slate-50 border border-slate-200 p-5 rounded-xl text-sm text-slate-700 shadow-sm print:break-inside-avoid">
            <h3 className="font-bold text-slate-900 mb-2 flex items-center gap-2">
                <AlertTriangle size={16} className="text-amber-500" />
                Hesaplama Mantığı
            </h3>
            <ul className="list-disc list-inside space-y-1.5 ml-1">
                <li><strong className="text-slate-900">Net Kâr Payı:</strong> Ortağın ofis gelirinden aldığı brüt paydan, ofis giderlerinden düşen payının (1/2) çıkarılmasıyla bulunur.</li>
                <li><strong className="text-slate-900">Cepten Ödenen Gider:</strong> Ortağın ofis kasası yerine kendi cebinden yaptığı harcamalardır.</li>
                <li><strong className="text-slate-900">Net Bakiye:</strong> Net Kâr Payı ile Cepten Ödenen Giderin toplamıdır. Pozitif ise ortak ofisten alacaklıdır, negatif ise ofise borçludur.</li>
            </ul>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden print:break-inside-avoid">
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="flex justify-between items-center py-2 border-b border-slate-50">
                    <span className={`text-sm font-medium ${altanStats.netProfitShare >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>Net Kâr Payı</span>
                    <span className={`font-bold ${altanStats.netProfitShare >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{formatCurrency(altanStats.netProfitShare)}</span>
                </div>
                ...
                <div className="mt-4 pt-4 border-t-2 border-slate-100">
                    <p className="text-sm text-slate-500 text-center mb-1">
                        {altanStats.totalBalance >= 0 ? 'Ofisten Alacaklı (Net Bakiye)' : 'Ofise Borçlu (Net Bakiye)'}
                    </p>
                    <p className={`text-3xl font-bold text-center ${altanStats.totalBalance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                        {formatCurrency(Math.abs(altanStats.totalBalance))}
                    </p>
                    <p className="text-xs text-slate-400 text-center mt-2">
                        {altanStats.totalBalance >= 0 
                            ? 'Bu tutar, ortağın net kâr payı ve cebinden ofis için ödediği giderlerin toplamıdır.' 
                            : 'Bu tutar, ortağın ofise ödemesi gereken net borcudur (Gider payı, gelir payından yüksek).'}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden print:break-inside-avoid">
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="flex justify-between items-center py-2 border-b border-slate-50">
                    <span className={`text-sm font-medium ${suatStats.netProfitShare >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>Net Kâr Payı</span>
                    <span className={`font-bold ${suatStats.netProfitShare >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{formatCurrency(suatStats.netProfitShare)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-50 bg-slate-50/50 px-2 rounded">
                    <span className="text-slate-700 text-sm flex items-center gap-1"><Wallet size={14}/> Cepten Ödediği Gider</span>
                ...
                <div className="mt-4 pt-4 border-t-2 border-slate-100">
                    <p className="text-sm text-slate-500 text-center mb-1">
                        {suatStats.totalBalance >= 0 ? 'Ofisten Alacaklı (Net Bakiye)' : 'Ofise Borçlu (Net Bakiye)'}
                    </p>
                    <p className={`text-3xl font-bold text-center ${suatStats.totalBalance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                        {formatCurrency(Math.abs(suatStats.totalBalance))}
                    </p>
                    <p className="text-xs text-slate-400 text-center mt-2">
                        {suatStats.totalBalance >= 0 
                            ? 'Bu tutar, ortağın net kâr payı ve cebinden ofis için ödediği giderlerin toplamıdır.' 
                            : 'Bu tutar, ortağın ofise ödemesi gereken net borcudur (Gider payı, gelir payından yüksek).'}
                    </p>
                </div>
