...
const PersonnelPage: React.FC<PersonnelProps> = ({ currentUser }) => {
  ...
  const [personnel, setPersonnel] = useState<Personnel[]>(() => {
    ...
    return storedPersonnel ? JSON.parse(storedPersonnel) : INITIAL_PERSONNEL;
  });
  const [payments, setPayments] = useState<SalaryPayment[]>(() => {
    const storedPayments = localStorage.getItem('emlak_salary_payments');
    return storedPayments ? JSON.parse(storedPayments) : INITIAL_SALARY_PAYMENTS;
  });
  const [expenses, setExpenses] = useState<Expense[]>(() => {
    const storedExpenses = localStorage.getItem('emlak_expenses');
    return storedExpenses ? JSON.parse(storedExpenses) : INITIAL_EXPENSES;
  });
  ...
  // Save Data
  useEffect(() => {
    localStorage.setItem('emlak_personnel', JSON.stringify(personnel));
  }, [personnel]);

  useEffect(() => {
    localStorage.setItem('emlak_salary_payments', JSON.stringify(payments));
  }, [payments]);
