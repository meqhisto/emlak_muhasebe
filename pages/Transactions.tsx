...
import { Plus, Search, User as UserIcon, FileText, CheckCircle2, Clock, Trash2, Banknote } from 'lucide-react';
...
const Transactions: React.FC<TransactionsProps> = ({ currentUser }) => {
  const [transactions, setTransactions] = useState<Transaction[]>(() => {
    ...
    return storedTrans ? JSON.parse(storedTrans) : INITIAL_TRANSACTIONS;
  });
  const [consultants] = useState<Consultant[]>(() => {
    const storedCons = localStorage.getItem('emlak_consultants');
    return storedCons ? JSON.parse(storedCons) : INITIAL_CONSULTANTS;
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    logAction('CREATE', `Yeni İşlem: ${newTransaction.propertyName} (${formatCurrency(newTransaction.totalRevenue)})`);
  ...
  const confirmDelete = () => {
    if (deletingTransactionId) {
      ...
      logAction('DELETE', `İşlem Silindi: ID ${deletingTransactionId}`);
  ...
  const handleConfirmPayment = (transactionId: string) => {
    ...
    const updatedTransactions = transactions.map(t => 
        t.id === transactionId ? { ...t, paymentStatus: PaymentStatus.PAID } : t
    );
    setTransactions(updatedTransactions);
    
    const transaction = transactions.find(t => t.id === transactionId);
    if (transaction) {
        logAction('APPROVE', `Hakediş Ödendi: ${transaction.propertyName}`);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={!!deletingTransactionId} onClose={cancelDelete} title="İşlemi Sil">
        <div className="space-y-4">
          <p className="text-slate-600">
            Bu işlemi silmek istediğinize emin misiniz? Bu işlem kalıcı olarak silinecek ve geri alınamaz.
          </p>
